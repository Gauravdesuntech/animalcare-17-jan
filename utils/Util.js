...
export const getFromAsyncStorage = async (STORAGE_KEY) => {
  try {
    let rawData = await AsyncStorage.getItem(STORAGE_KEY);
    return rawData !== null ? await JSON.parse(rawData) : null;
  } catch (e) {
    throw new Error("failed data retrieve from device");
  }
};

export const setToAsyncStorage = async (STORAGE_KEY, value) => {
  try {

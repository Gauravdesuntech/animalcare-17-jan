...
export default class AddUser extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      site_code:
        typeof props.route.params !== "undefined"
          ? props.route.params.siteCode
          : undefined,
      site_name:
        typeof props.route.params !== "undefined"
          ? props.route.params.siteName
          : undefined,
      desgCode:
        typeof props.route.params !== "undefined"
    ...
    Promise.all(methods)
      .then((response) => {
        ...
        if (parseInt(this.state.id) > 0) {
          ...
          console.log(userData.qr_code_image);
  ...
  toggleSiteMenu = () =>
    this.setState({ isSiteMenuOpen: !this.state.isSiteMenuOpen });
  ...
  loadSiteLocation = (dept_code) => {
    let cid = this.context.userDetails.cid;
    getSiteLocation(cid, dept_code)
      .then((data) => {
        this.setState({
          isLoading: false,
          siteLocation: data.map((item) => {
            return {
              id: item.id,
              name: item.site_name,
      ...
      () => {
        ...
        // }
          ...
          if (this.state.site_code) {
            reqObj.site_code = this.state.site_code
          }
          if (parseInt(id) === 0) {
      ...
      <View style={[globalStyles.container, globalStyles.p5]}>
        <ScrollView ref={this.formScrollViewRef} nestedScrollEnabled>
          ...
          <View style={globalStyles.boxBorder}>
            ...
            {(this.state.siteLocation.length == 0) && (this.state.site_name == "Null" || !this.state.site_name) ? null :
              <View>
                <InputDropdown
                  label={"Site Location"}
                  value={this.state.site_name}
                  isOpen={this.state.isSiteMenuOpen}
                  items={this.state.siteLocation}
                  openAction={this.toggleSiteMenu}
                  closeAction={this.toggleSiteMenu}
                  setValue={this.setSiteData}
                  labelStyle={globalStyles.labelName}
                  textFieldStyle={globalStyles.textfield}
                  style={globalStyles.fieldBox}
                />

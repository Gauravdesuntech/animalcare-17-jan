...
export default class FeedingSectionMenu extends React.Component {
    ...
    ImagePicker.requestMediaLibraryPermissionsAsync().then((status) => {
      if (status.granted) {
        ...
        ImagePicker.launchImageLibraryAsync(optins).then(async (result) => {
          if (!result.cancelled) {
            console.log(await FileSystem.getInfoAsync(result.uri));
            this.setState({
              Images: [{ id: Number(this.state.imageID), uri: result.uri }],
  ...
  searchData = () => {
    ...
    let userData = sectionsData.filter((item) => {
      let user = item.users.filter((user) => {
        ...
        let index =
          name == undefined ? -1 : name.indexOf(searchValue.toLowerCase());
        return index > -1;
      });
    ...
    this.setState({
      isSearching: false,
      isModalOpen: false,
      searchData: userData.length > 0 ? userData : data,
    });
  };

  gotoItems = async (item, feed) => {
  ...
  handleMarkasComplete = async () => {
    ...
    if (this.state.imagebase64) {
      ...
      insert_data("feeding_images", asyncObj);
    }
    updateFeedWork(obj)
      .then((res) => {
        this.setState(
          ...
          () => {
            this.loadSections(this.state.today);
            alert("Upload Successfully Done without Image!!");
          }
        );
      })
      .catch((err) => {
                              ...
                              onPress={
                                feed.task[0].status == "completed"
                                  ? () => {
                                      ...
                                      if (
                                        JSON.parse(feed.task[0].proof_images)
                                          .length > 0
                                      ) {
  ...
  imageSync = async () => {
    ...
    // delete_table_data("feeding_images");
    // return;
    get_data("feeding_images")
      .then((response) => {
        // console.log("Responsew>>>>>>>>>>>>>", response);
        // return
        if (response.length > 0) {
          let obj = {
            feedWork: JSON.stringify(response),
          };
          syncUpdateFeedWork(obj)
            .then((res) => {
              console.log({ res });
              delete_table_data("feeding_images");
              alert("Images sync done!!");
            })
            .catch((err) => {
              alert("Something went wrong!! Try again!");
              this.setState({ isLoading: false });
      ...
      <Container>
        ...
        {this.state.isLoading ? (
          ...
          <>
            ...
            <Text style={[globalStyles.labelName, { textAlign: "center" }]}>
              Please Sync Images before 7PM !!
            </Text>
            <FlatList
              ListEmptyComponent={() => <ListEmpty />}
              data={
